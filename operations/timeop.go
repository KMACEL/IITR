...
retry:
	/*if (timop.GetHour() == 10 && timop.GetMinute() == 10) || (timop.GetHour() == 17 && timop.GetMinute() == 10) {
		minuteValue = timop.GetMinute()
		getLogFlag = true
	}*/

	if (timop.GetHour() == 8 && timop.GetMinute() == 13) || (timop.GetHour() == 17 && timop.GetMinute() == 05) {
		minuteValue = timop.GetMinute()
		getDeviceRaportFlag = true
	}

	if minuteValue == timop.GetMinute() {
		time.Sleep(2 * 60 * time.Second)
		goto retry
	}

	if getLogFlag {
		go cases.GetLogDevices{}.Start(getLogList)
		getLogFlag = false
	}

	if getDeviceRaportFlag {
		//go cases.ApplicationStatus{}.Start("ApplicationPackage_"+timop.GetTimeNamesFormat()+".xlsx", "tr.com.innology.taksipager", "com.ardic.android.iot.appblocker", "com.android.launcher3", "com.google.android.apps.maps", "com.streamaxtech.mdvr.direct")
		getDeviceRaportFlag = false
	}
